...
out = csv.writer(f)
out.writerow(["frame", "gates", "train", "tid", "ioa"])
...
while True:
    ...
    train = int(any(box(*b).intersects(zone) for b, c in zip(boxes, clss) if c == TRAIN))
    ...
    for tid, b in people:
        ...
        out.writerow([idx, gates, train, tid, round(p.intersection(zone).area / p.area, 4)])
