...
while True:
    ...
    g = m2(frame, verbose=False, classes=[0,1])[0]        # model 2 detections
    for d in g.boxes:
        x1, y1, x2, y2 = map(int, d.xyxy[0].tolist())
        cv2.rectangle(frame, (x1, y1), (x2, y2), (255, 0, 255), 2)
        cv2.putText(frame, f"{g.names[int(d.cls)]} {float(d.conf):.2f}",
                    (x1, max(14, y1 - 6)),
    ...
    for f0, tid, pet, cx, cy, n in exits.get(idx, []):    # persistent exit markers
        ...
        cv2.putText(frame, str(tid), (cx + 8, ly),
